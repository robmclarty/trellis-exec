...
// Fixtures
// ---------------------------------------------------------------------------
function makeValidTasksJson(overrides) {
    return {
        projectRoot: ".",
        specRef: "./spec.md",
        planRef: "./plan.md",
...
function makeSuccessParseResult(tasksJson) {
    return {
        success: true,
        tasksJson,
        enrichmentNeeded: [],
...
function makeFailedParseResult() {
    return {
        success: false,
        tasksJson: null,
        enrichmentNeeded: [],
...
describe("compilePlan", () => {
    let tmpDir;
    const mockParsePlan = parsePlan;
    const mockEnrichPlan = enrichPlan;
    beforeEach(() => {
    ...
    describe("parser success path", () => {
        it("parses, enriches, and writes output", async () => {
            ...
            const written = JSON.parse(readFileSync(join(tmpDir, "output", "tasks.json"), "utf-8"));
