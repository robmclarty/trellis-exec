...
describe("detectWebApp", () => {
    ...
    it("returns true when package.json has react dependency", () => {
        const d = tmp();
        writeFileSync(join(d, "package.json"), JSON.stringify({ dependencies: { react: "^18.0.0", "react-dom": "^18.0.0" } }));
        expect(detectWebApp(d)).toBe(true);
    });
    it("returns true when package.json has vue devDependency", () => {
        const d = tmp();
        writeFileSync(join(d, "package.json"), JSON.stringify({ devDependencies: { vue: "^3.0.0" } }));
        expect(detectWebApp(d)).toBe(true);
    });
    it("returns false for backend-only project", () => {
        const d = tmp();
        writeFileSync(join(d, "package.json"), JSON.stringify({ dependencies: { express: "^4.0.0", pg: "^8.0.0" } }));
        expect(detectWebApp(d)).toBe(false);
    });
    it("returns false for CLI tool project", () => {
        const d = tmp();
        writeFileSync(join(d, "package.json"), JSON.stringify({ dependencies: { commander: "^10.0.0" } }));
        expect(detectWebApp(d)).toBe(false);
    });
    // ---
...
//# sourceMappingURL=detectWebApp.test.js.map