...
import { detectDevServerCommand, startDevServer } from "../devServer.js";
...
describe("detectDevServerCommand", () => {
    ...
    it("prefers scripts.dev over scripts.start", () => {
        const dir = makeTempDir();
        try {
            writeFileSync(join(dir, "package.json"), JSON.stringify({ scripts: { dev: "vite", start: "node server.js" } }));
            expect(detectDevServerCommand(dir)).toBe("npm run dev");
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
