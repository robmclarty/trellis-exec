 ...
 */
export function detectDevServerCommand(projectRoot) {
...
async function waitForReady(child, getDetectedPort, hasExited, getExitError, timeout) {
...
async function pollUntilReady(port, deadline, child, hasExited, getExitError) {
    while (Date.now() < deadline) {
        if (hasExited()) {
            throw new Error(getExitError() ?? "Dev server process exited before becoming ready");
        }
