 ...
 */
export async function runBrowserAcceptance(config) {
    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
        ...
        const testerInstructions = buildTesterPrompt(specContent, devUrl, testOutputDir, attempt > 0 ? lastResults : undefined);
        ...
        console.log(`Browser acceptance: ${failCount} criteria failed.`);
...
function buildTesterPrompt(specContent, devUrl, testOutputDir, previousResults) {
    const lines = [];
...
function parseTesterOutput(output) {
