...
import { buildRejudgePrompt, buildReferenceContext, buildPhaseContext, buildFixPrompt, formatIssue, normalizeReport, parseJudgeResult, collectLearnings, } from "../prompts.js";
...
describe("formatIssue", () => {
    ...
    it("returns task: description when task is present", () => {
        expect(formatIssue({ task: "task-1-1", description: "missing file" })).toBe("task-1-1: missing file");
    });
});
// ---------------------------------------------------------------------------
...
describe("normalizeReport", () => {
    ...
    it("uses handoffBriefing as fallback for handoff", () => {
        const report = normalizeReport({ handoffBriefing: "next phase context" }, "phase-1");
    ...
    beforeEach(() => {
        mockedReadFileSync.mockImplementation((path) => {
            if (String(path).endsWith("spec.md"))
                return "# The Spec\nSpec content here.";
            if (String(path).endsWith("guidelines.md"))
                return "# Guidelines\nGuideline content.";
            return "";
        });
    });
