...
import { initState, loadState, saveState, updateStateAfterPhase, updateTaskStatus, applyReportToTasks, applyCorrections, getPhaseCommitRange, } from "../stateManager.js";
...
function makePhaseReport(phaseId) {
    return {
        phaseId,
        status: "complete",
...
describe("stateManager", () => {
    ...
    describe("loadState", () => {
        ...
        it("throws for invalid schema", () => {
            ...
            writeFileSync(statePath, JSON.stringify({ currentPhase: 123 }), "utf-8");
            expect(() => loadState(statePath)).toThrow();
        });
    });
    describe("updateStateAfterPhase", () => {
        ...
        it("sets currentPhase to empty string when last phase completes", () => {
            ...
            state = updateStateAfterPhase(state, makePhaseReport("phase-1"), tasks.phases);
            state = { ...state, currentPhase: "phase-2" };
            state = updateStateAfterPhase(state, makePhaseReport("phase-2"), tasks.phases);
    ...
    describe("updateTaskStatus", () => {
        ...
        it("throws for unknown phase", () => {
            const tasks = makeTasksJson();
            expect(() => updateTaskStatus(tasks, "phase-99", "task-1-1", "complete")).toThrow(/Phase not found/);
        });
        it("throws for unknown task", () => {
            const tasks = makeTasksJson();
            expect(() => updateTaskStatus(tasks, "phase-1", "task-99", "complete")).toThrow(/Task not found/);
        });
    });
