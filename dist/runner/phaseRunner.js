...
export function buildPhaseContext(phase, state, handoff, ctx) {
    ...
    lines.push(`(Pre-loaded from \`${basename(ctx.specPath)}\`. Also available via readFile('${basename(ctx.specPath)}').)`);
    lines.push("");
    try {
    ...
    if (ctx.guidelinesPath) {
        lines.push(`(Pre-loaded from \`${basename(ctx.guidelinesPath)}\`. Also available via readFile('${basename(ctx.guidelinesPath)}').)`);
        lines.push("");
        try {
