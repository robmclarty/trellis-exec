...
import { initState, loadState, saveState, updateStateAfterPhase, applyReportToTasks, applyCorrections, } from "./stateManager.js";
...
import { REPORT_FILENAME, collectLearnings, buildPhaseContext, normalizeReport, formatIssue, parseJudgeResult, buildJudgePrompt, buildRejudgePrompt, buildFixPrompt, buildReporterPrompt, } from "./prompts.js";
/**
...
export function reviewPhaseContract(phase) {
    ...
    for (const task of phase.tasks) {
        ...
        if (task.id.includes("-corrective-"))
            continue;
...
// ---------------------------------------------------------------------------
async function runBrowserSmokeForPhase(ctx, phase, projectRoot) {
...
// ---------------------------------------------------------------------------
async function runEndOfBuildAcceptance(ctx, tasksJson, projectRoot) {
...
export async function runPhases(ctx, tasksJson) {
    ...
    try {
        ...
        while (phaseIndex < mutableTasksJson.phases.length) {
            ...
            if (report.status === "complete" || report.status === "partial") {
                const verification = verifyCompletion(projectRoot, correctedPhase, report, report.startSha);
    ...
    if (phasesFailed.length === 0) {
        browserAcceptanceReport = await runEndOfBuildAcceptance(localCtx, mutableTasksJson, projectRoot) ?? undefined;
    }
    return {
