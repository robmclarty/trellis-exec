...
function makeLauncherConfig(overrides) {
    return {
...
describe("createAgentLauncher", () => {
    describe("dispatchSubAgent", () => {
        ...
        it("returns configured mock response by agent type", async () => {
            ...
            const launcher = createAgentLauncher(makeLauncherConfig({
                mockResponses: new Map([["implementer", mockResult]]),
            }));
            const result = await launcher.dispatchSubAgent(makeSubAgentConfig({ type: "implementer" }));
