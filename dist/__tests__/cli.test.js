...
describe("handleCompile (subprocess)", () => {
    ...
    it("succeeds with deterministic parse and writes tasks.json", () => {
        ...
        const stdout = execSync(`node ${cliPath} compile ${join(tmpDir, "plan.md")} --spec ${join(tmpDir, "spec.md")} --output ${outputPath}`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
    ...
    it("exits with error when plan file does not exist", () => {
        let exitCode = 0;
        try {
            execSync(`node ${cliPath} compile /nonexistent/plan.md --spec /nonexistent/spec.md`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
        }
        catch (err) {
            exitCode = err.status;
        }
...
describe("handleRun (subprocess)", () => {
    ...
    it("--dry-run prints report and exits 0", () => {
        const { tmpDir, tasksJsonPath } = createTempTasksJson();
        trackTmpDir(tmpDir);
        const stdout = execSync(`node ${cliPath} run ${tasksJsonPath} --dry-run`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
        expect(stdout).toContain("phase-1");
    ...
    it("exits with error when tasks.json does not exist", () => {
        let exitCode = 0;
        try {
            execSync(`node ${cliPath} run /nonexistent/tasks.json --dry-run`, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
        }
        catch (err) {
            exitCode = err.status;
        }
