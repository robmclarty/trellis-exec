...
import { mkdtempSync, rmSync, writeFileSync, readFileSync, mkdirSync, cpSync, existsSync, } from "node:fs";
...
// ---------------------------------------------------------------------------
const FIXTURES_DIR = resolve(import.meta.dirname ?? new URL(".", import.meta.url).pathname, "../../test/fixtures/e2e");
...
import { resolveExecutionOrder, detectTargetPathOverlaps, } from "../runner/scheduler.js";
 ...
 */
async function compileSamplePlan() {
...
function hasClaude() {
    if (!process.env["TRELLIS_E2E_CLAUDE"])
        return false;
    try {
...
describe("e2e integration tests", () => {
    ...
    describe("state round-trip: run, persist, resume", () => {
        it("saves state after phase 1, resumes and completes phase 2", async () => {
            ...
            const phase1ReportFromState = state.phaseReports.find((r) => r.phaseId === "phase-1");
    ...
    describe("handoff from phase 1 consumed by phase 2", () => {
        it("handoff string persists in state and is available to phase 2", async () => {
            const tasksJson = await compileSamplePlan();
            tmpDir = setupTmpDir(tasksJson);
            const handoffText = "# Phase 1 → Phase 2 Handoff\n\n## What was built\n- greet.ts with greet() function";
            const reports = new Map([
                [
                    "phase-1",
            ...
            const stateRaw = readFileSync(join(tmpDir, "state.json"), "utf-8");
            const state = JSON.parse(stateRaw);
            const phase1Report = state.phaseReports.find((r) => r.phaseId === "phase-1");
    ...
    describe("REPL output truncation", () => {
        it("truncates output exceeding 8192 chars with marker", async () => {
            ...
            const { createReplSession: realCreateReplSession } = await vi.importActual("../orchestrator/replManager.js");
            ...
            try {
                ...
                const result = await session.eval('console.log("x".repeat(10000))');
                expect(result.success).toBe(true);
    ...
    describe("architectural validation", () => {
        it("phaseRunner.ts has zero direct LLM imports", () => {
            const phaseRunnerSource = readFileSync(resolve(import.meta.dirname ?? ".", "../runner/phaseRunner.ts"), "utf-8");
    ...
    describe.skipIf(!hasClaude())("full end-to-end with claude CLI", () => {
        it("compiles plan and runs against test project", { timeout: 600_000 }, async () => {
            ...
            const planContent = readFileSync(join(projectDir, "sample-plan.md"), "utf-8");
            const parseResult = parsePlan(planContent, "./sample-spec.md", "./sample-plan.md");
            expect(parseResult.success).toBe(true);
            const mockEnricher = async () => JSON.stringify({ resolved: [] });
            ...
            execSync(`node ${cliPath} run ${tasksJsonPath} --headless --isolation none`, {
                cwd: projectDir,
                timeout: 300_000,
                stdio: "pipe",
                encoding: "utf-8",
