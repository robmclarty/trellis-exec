...
export function ensureAuthVolume(volumeName: string): void {
  try {
 ...
 */
export function cleanAuthVolume(volumeName: string, image: string): void {
  ...
  try {
    execSync(
      `docker run --rm --user root -v ${volumeName}:/home/claude/.claude --entrypoint sh ${image} -c '${script}'`,
      { stdio: "pipe" },
    );
  } catch {
...
export function extractAuthToken(
  ...
  try {
    const result = execSync(
      `docker run --rm -v ${volumeName}:/home/claude/.claude --entrypoint sh ${image} -c 'cat /home/claude/.claude/_claude.json 2>/dev/null'`,
 ...
 */
export function generateContainerSettings(
  hostSettingsPath: string,
): string | undefined {
  if (!existsSync(hostSettingsPath)) return undefined;
 ...
 */
export function buildAuthMounts(opts: AuthMountOptions): string[] {
  const args: string[] = [];
 ...
 */
export function runContainerLogin(
  image: string,
  volumeName: string,
): boolean {
  ...
  try {
    execSync(
      `docker run --rm -v ${volumeName}:/home/claude/.claude --entrypoint sh ${image} -c 'test -s /home/claude/.claude/_claude.json'`,
      { stdio: "pipe" },
    );
    return true;
  } catch {
