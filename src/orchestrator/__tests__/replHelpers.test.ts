...
function makeHelpers(
  ...
  overrides?: {
    statePath?: string;
    agentLauncher?: ((config: SubAgentConfig) => Promise<SubAgentResult>) | null;
  },
): ReplHelpers {
...
describe("createReplHelpers", () => {
  ...
  describe("readFile", () => {
    ...
    it("throws on path traversal with ../", () => {
      expect(() => helpers.readFile("../../../etc/passwd")).toThrow(
  ...
  describe("writeFile", () => {
    ...
    it("throws on path traversal with ../", () => {
      expect(() => helpers.writeFile("../../../tmp/escape.txt", "bad")).toThrow(
        "Path is outside project root",
      );
    });
  ...
  describe("listDir", () => {
    ...
    it("throws on path traversal", () => {
      expect(() => helpers.listDir("../../")).toThrow(
        "Path is outside project root",
      );
    });
  ...
  describe("dispatchSubAgent", () => {
    ...
    it("delegates to agentLauncher when provided", async () => {
      const mockLauncher = async (_config: SubAgentConfig): Promise<SubAgentResult> => ({
        success: true,
        output: "launched",
    ...
    it("passes config to agentLauncher correctly", async () => {
      let receivedConfig: SubAgentConfig | undefined;
      const mockLauncher = async (config: SubAgentConfig): Promise<SubAgentResult> => {
    ...
    it("returns error when passed a string instead of object", async () => {
      const result = await helpers.dispatchSubAgent(
        "implement" as unknown as SubAgentConfig,
      );
    ...
    it("returns error when required fields are missing", async () => {
      const result = await helpers.dispatchSubAgent(
        { type: "implement" } as unknown as SubAgentConfig,
      );
      expect(result.success).toBe(false);
    ...
    it("returns error when passed null", async () => {
      const result = await helpers.dispatchSubAgent(
        null as unknown as SubAgentConfig,
      );
      expect(result.success).toBe(false);
      expect(result.error).toContain("requires an object");
    });
