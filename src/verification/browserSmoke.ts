...
let _browsersInstalled: boolean | null = null;
 ...
 */
async function areBrowsersInstalled(): Promise<boolean> {
  if (_browsersInstalled !== null) return _browsersInstalled;
  try {
...
function makeSkippedReport(reason: string): BrowserSmokeReport {
  return {
    passed: true,
    skipped: true,
    reason,
 ...
 */
export async function runBrowserSmoke(config: BrowserSmokeConfig): Promise<BrowserSmokeReport> {
  ...
  if (!(await areBrowsersInstalled())) {
    console.log(
      "Playwright installed but browsers not found. " +
  ...
  try {
    ...
    const hasContent = await page.evaluate(() => {
      ...
      return selectors.some((s: string) => document.querySelector(s) !== null);
    });
    ...
    const interactiveElements = await page.$$(
      "button, a[href]:not([href^='http']):not([href^='//']), input, select, textarea, [role='button']",
    );
    ...
    for (let i = 0; i < toTest.length; i++) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const el = toTest[i] as any;
      try {
        const isVisible = await el.isVisible();
        if (!isVisible) continue;

        const tagName: string = await el.evaluate((e: Element) => e.tagName.toLowerCase());
        const label: string = await el.evaluate((e: Element) =>
          (e as HTMLElement).textContent?.trim().slice(0, 50) ??
        ...
        if (["input", "select", "textarea"].includes(tagName)) {
          try {
            ...
            interactionFailures.push(
              `Failed to focus ${tagName}[${label}]: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
          continue;
        }
        ...
        const desc: string = await el.evaluate((e: Element) =>
          (e as HTMLElement).textContent?.trim().slice(0, 50) ?? e.tagName,
        ).catch(() => `element[${i}]`);
        interactionFailures.push(
          `Click failed on "${desc}": ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
